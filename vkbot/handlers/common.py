...
async def try_help(_bot, message, _state, text, _uid) -> bool:
    ...
    await message.answer(
        ...
        "«📖 Вся неделя» покажет все дни сразу.\n"
        "«👀 Другое расписание» — посмотреть любой курс и направление; твой выбор при этом не меняется.\n"
        "«🔄 Сменить курс/направление» — наоборот, меняет сохранённый выбор.\n"
        ...
        "🔔 Подписки на пары\n"
        "Кнопка «🔔 Подписки на пары» есть прямо в главном меню.\n"
        "Добавь курс и направление — бот будет напоминать за 10 минут до начала каждой пары.\n"
        "Чтобы отписаться — нажми «❌» на нужной подписке в том же разделе.\n\n"
        "📌 Дедлайны\n"
        ...
        "🔗 Связать аккаунты\n"
        "Склейка VK и Telegram: общие подписки и заметки, пуши в оба мессенджера.\n"
        "Код из одного бота вводится в другом.\n\n"
        "🗑 Удалить мои данные\n"
        "В «⚙️ Прочее» — полное удаление заметок, подписок, согласия и сессий.\n"
        "Потребуется подтверждение; восстановить будет нельзя.\n\n"
