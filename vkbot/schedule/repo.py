...
class ScheduleRepo:
    ...
    def get_day(self, course: int, direction: str, day: str, week_type: str) -> str:
        ...
        # Фильтр чётности совпадает с workers/classes._day_rows: пустой/NULL —
        # каждую неделю; legacy-префикс «[чёт]/»/«[нечет]» в subject отсекаем,
        # иначе показ дня и пуши расходятся на базах из эпохи Telegram-бота.
        other = "нечет" if week_type == "чёт" else "чёт"
        with connect(config.SCHEDULE_DB) as conn:
            rows = conn.execute(
                """
                ...
                """,
                (course, direction, day, week_type, "[" + other + "]%"),
            ).fetchall()
