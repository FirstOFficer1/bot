...
условиями. Сервис предоставляет {{ developer_name }} (далее — Оператор);
связаться с ним можно через <a href="{{ support_url }}">сообщество бота</a>
ВКонтакте.</p>
...
<p>Сервис предоставляется «как есть». Оператор прилагает усилия для точности
расписания, но не гарантирует отсутствие ошибок и не несёт ответственности за
решения, принятые на основе данных Сервиса.</p>
...
def terms():
    # Как и политика — шаблон: оператор должен быть назван в самом приложении
    # (типовое соглашение VK, п. 1.3), а имя приходит из .env.
    body = render_template_string(
        _TERMS_BODY, developer_name=DEVELOPER_NAME, support_url=SUPPORT_URL,
    )
    return render_template_string(
